...
namespace Kedacom.AutoGen.CppSharpUnsafeGenerator
{
    internal static class FunctionExportHelper
    {
        ...
        public static IEnumerable<T> WhereIf<T>(this IEnumerable<T> array, bool where, Func<T, bool> predicate)
        {
            if (where) return array.Where(predicate);
            return array;
        }
        public static IEnumerable<FunctionExport> LoadFunctionExports(string path, string searchPattern="*.dll", Func<string, bool> exportFilter=default)
        {
            var libraries = Directory.EnumerateFiles(path, searchPattern);
            foreach (var libraryPath in libraries)
            {
                var libraryName = Path.GetFileNameWithoutExtension(libraryPath);
                var libraryVersion = 7;

                var exports = GetExports(libraryPath);
                foreach (var export in exports.WhereIf(exportFilter!=null,exportFilter)) yield return new FunctionExport { LibraryName = libraryName, LibraryVersion = libraryVersion, Name = export};
            }
        }
